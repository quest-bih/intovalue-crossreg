...
**Proposal:** text in `comment_summary_results` seems to be more consistent when signalling no results: for EUCTR it says "euctr no results", for DRKS it says "drks no results", and for ClinicalTrials.gov it says "ctgov no results". In these two new variables, because we want to include everything that they linked as TRUE and only no results as FALSE, I propose to create these columns based on the no results text.
...
#### Replicate analysis of summary results
...
#### Replicate figure of summary results
...
#combine plots
all_combined_plot <- plot_clinicaltrials + plot_drks + plot_layout(ncol = 2)
...
### Overall plot (without distinguishing DRKS and ClinicalTrials.gov)
...
#### Overall plot (without distinguishing between CTgov and DRKS)
