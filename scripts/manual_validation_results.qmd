...
main_data_summary <- discrepancy_data |>
  ...
  mutate(result_label = case_when(
      ...
      has_summary_results_reg1_main == TRUE & has_summary_results_reg2_main == FALSE ~ "Only in EUCTR",
      has_summary_results_reg1_main == TRUE & has_summary_results_reg2_main == TRUE ~ "Both",
      has_summary_results_reg1_main == FALSE & has_summary_results_reg2_main == TRUE ~ "Only in other registry"),
    result_label = factor(result_label, levels = c("Only in other registry", "Both", "Only in EUCTR", "No results"))) |>
...
sensitivity_data_summary <- discrepancy_data |>
  ...
  mutate(result_label = case_when(
      ...
      has_summary_results_reg1_sensitivity == TRUE & has_summary_results_reg2_sensitivity == FALSE ~ "Only in EUCTR",
      has_summary_results_reg1_sensitivity == TRUE & has_summary_results_reg2_sensitivity == TRUE ~ "Both",
      has_summary_results_reg1_sensitivity == FALSE & has_summary_results_reg2_sensitivity == TRUE ~ "Only in other registry"),
    result_label = factor(result_label, levels = c("Only in other registry", "Both", "Only in EUCTR", "No results"))) |>
  group_by(registry2, result_label) |>
  summarise(count = n(), .groups = 'drop') |>
  group_by(registry2) |>
...
plot_crossregs_freq <- function(tib_summary, registry2, legend_position = "none") {
  ...
  tib_summary |> 
    ...
    labs(x = paste("EUCTR -",  registry2), y = "Cross-registered pairs", fill = "Results reporting") +
    scale_fill_manual(values = c("No results" = "lightgray", 
                                 "Only in EUCTR" = "#2577c5", 
                                 "Both" = "#3594cc", 
                                 "Only in other registry" = "#8cc5e3")) +
...
plot1 <- bind_rows(
  main_data_summary |> 
    ...
    mutate(analysis_type = "Tabular results only"), 
  sensitivity_data_summary |> 
    filter(registry2 == "ClinicalTrials.gov") |> 
    mutate(analysis_type = "Tabular + other formats (EUCTR)")) |> 
  mutate(analysis_type = factor(analysis_type, levels = c("Tabular results only", "Tabular + other formats (EUCTR)"))) |> 
...
plot2 <- bind_rows(
  ...
  sensitivity_data_summary |> 
    filter(registry2 == "DRKS") |> 
    mutate(analysis_type = "Tabular + other formats")) |> 
  mutate(analysis_type = factor(analysis_type, levels = c("Tabular results only", "Tabular + other formats"))) |> 
  plot_crossregs_freq(registry2 = "DRKS", legend_position = "right")

# Define custom labels with manual line breaks
custom_labels <- c(
  "Tabular results only" = "Tabular results\nonly",
  "Tabular + other formats" = "Tabular +\nother formats",
  "Tabular + other formats (EUCTR)" = "Tabular + \nother formats (EUCTR)",
  "Publications in ClinicalTrials.gov" = "Publications in\nClinicalTrials.gov")

# Modify each plot before combining
plot1 <- plot1 +
  scale_x_discrete(labels = custom_labels) + 
  theme(legend.position = "none")

plot2 <- plot2 + scale_x_discrete(labels = custom_labels)

# Combine plots
combined_plot_summary <- plot1 + plot2 + plot_layout(ncol = 2)
print(combined_plot_summary)

#ggsave("summary_results_main.png", 
#       plot = combined_plot_summary, 
...
sensitivity_data_summary_v2 <- discrepancy_data |>
  ...
  mutate(result_label = case_when(
      ...
      has_summary_results_reg1_sensitivity_v2 == TRUE & has_summary_results_reg2_sensitivity_v2 == FALSE ~ "Only in EUCTR",
      has_summary_results_reg1_sensitivity_v2 == TRUE & has_summary_results_reg2_sensitivity_v2 == TRUE ~ "Both",
      has_summary_results_reg1_sensitivity_v2 == FALSE & has_summary_results_reg2_sensitivity_v2 == TRUE ~ "Only in other registry"),
    result_label = factor(result_label, levels = c("Only in other registry", "Both", "Only in EUCTR", "No results"))) |>
  group_by(registry2, result_label) |>
  summarise(count = n(), .groups = 'drop') |>
  group_by(registry2) |>
...
# Create plot
summary_results_sens <- bind_rows(
  main_data_summary |> 
    filter(registry2 == "ClinicalTrials.gov") |>  
    mutate(analysis_type = "Tabular results only"), 
  sensitivity_data_summary |> 
  ...
  sensitivity_data_summary_v2 |>
    filter(registry2 == "ClinicalTrials.gov") |>
    mutate(analysis_type = "Publications in ClinicalTrials.gov")) |> 
  mutate(analysis_type = factor(analysis_type, levels = c("Tabular results only", "Tabular + other formats (EUCTR)", "Publications in ClinicalTrials.gov"))) |> 
  plot_crossregs_freq(registry2 = "ClinicalTrials.gov", legend_position = "right")
...
# Modify each plot before combining
summary_results_sens <- summary_results_sens +
  scale_x_discrete(labels = custom_labels)

# Result
print(summary_results_sens)

#ggsave("summary_results_sens.png", 
#      plot = summary_results_sens, 
...
This analysis was also limited to confirmed cross-registrations with an available DE protocol (6 cases were excluded):

```{r}

recruitment_data <- discrepancy_data |>
  filter(!trn1 %in% c("2011-006041-14", "2012-002699-14", "2009-013328-21", 
                      "2005-003018-14", "2012-004555-36", "2005-002011-24"))
```
...
Since there is no missing data on either registry, the analysis was done on `r nrow(recruitment_data)` pairs of true cross-registered pairs.
...
Of those `r nrow(recruitment_data)` pairs, `r nrow(recruitment_data |> filter(registry2 == "ClinicalTrials.gov"))` were EUCTR-ClinicalTrials.gov and `r nrow(recruitment_data |> filter(registry2 == "DRKS"))` were EUCTR-DRKS
...
euctr_ctgov_percent_match <- recruitment_data |> 
  filter(registry2 == "ClinicalTrials.gov") |> 
  summarize(match_percentage = mean(overall_recruitment_status_reg1 == overall_recruitment_status_reg2) * 100) |>
  round(0)

euctr_ctgov_count_match <- recruitment_data |> filter(registry2 == "ClinicalTrials.gov", overall_recruitment_status_reg1 == overall_recruitment_status_reg2)
  
euctr_ctgov_percent_not_match <- recruitment_data |> 
  filter(registry2 == "ClinicalTrials.gov") |> 
  summarize(match_percentage = mean(overall_recruitment_status_reg1 != overall_recruitment_status_reg2) * 100) |>
  round(0)

euctr_ctgov_count_not_match <- recruitment_data |> filter(registry2 == "ClinicalTrials.gov", overall_recruitment_status_reg1 != overall_recruitment_status_reg2)

# EUCTR-DRKS 
euctr_drks_percent_match <- recruitment_data |> 
  filter(registry2 == "DRKS") |> 
  summarize(match_percentage = mean(overall_recruitment_status_reg1 == overall_recruitment_status_reg2) * 100) |>
  round(0)

euctr_drks_count_match <- recruitment_data |> filter(registry2 == "DRKS", overall_recruitment_status_reg1 == overall_recruitment_status_reg2)
  
euctr_drks_percent_not_match <- recruitment_data |> 
  filter(registry2 == "DRKS") |> 
  summarize(match_percentage = mean(overall_recruitment_status_reg1 != overall_recruitment_status_reg2) * 100) |>
  round(0)

euctr_drks_count_not_match <- recruitment_data |> filter(registry2 == "DRKS", overall_recruitment_status_reg1 != overall_recruitment_status_reg2)
```
...
completion_date_main <- discrepancy_data |>
  filter(overall_recruitment_status_reg1 == "Completed" & overall_recruitment_status_reg2 == "Completed") |>
  ...
  geom_abline(slope = 1, intercept = 0, linetype = "dotted", color = "black", alpha = 0.7) +
  theme_classic() +
  theme(legend.position = "right",
        legend.background = element_rect(color = "black", fill = "white"),
        legend.text = element_text(size = 14),    # Size of legend categoriews
        legend.title = element_text(size = 15),   # Size of legend title
        axis.text = element_text(size = 12),      # Size of years
        axis.title = element_text(size = 14),     # Size of axis titles
        axis.ticks.length = unit(0.3, "cm"),      # Length of axis ticks
        axis.title.x = element_text(margin = margin(t = 20, b = 15)), # Size of margins x-axis
        axis.title.y = element_text(margin = margin(r = 20, l = 10)), # Size of margins y-axis
        axis.text.x = element_text(margin = margin(t = 10)), # Distance of label to x-axis
        axis.text.y = element_text(margin = margin(r = 10))  # Distance of label to y-axis
  ) +  
  ...
  scale_color_manual(name = "Registry", values = c("#F7C72F", "steelblue")) +
  coord_fixed() # Ensures 1:1 ratio for the plot

print(completion_date_main)

#ggsave("completion_date_main.png", 
#       plot = completion_date_main, 
...
completion_date_sens <- discrepancy_data |>
  filter(overall_recruitment_status_reg1 == "Completed" & overall_recruitment_status_reg2 == "Completed") |>
...
print(completion_date_sens)

#ggsave("completion_date_sens.png", 
#       plot = completion_date_sens, 
#       width = 12, height = 10, dpi = 300)
```
